...
if grep -q "com.apple.developer.applesignin" "Khandoba Secure Docs/Khandoba_Secure_Docs.entitlements"; then
    success "Apple Sign In enabled"
else
    error "Apple Sign In not configured"
fi

if grep -q "iCloud.com.khandoba.securedocs" "Khandoba Secure Docs/Khandoba_Secure_Docs.entitlements"; then
...
for key in "${REQUIRED_KEYS[@]}"; do
    if grep -q "$key" "Khandoba Secure Docs/Info.plist"; then
        success "$key present"
    else
...
if grep -q "com.khandoba" "Khandoba Secure Docs.xcodeproj/project.pbxproj"; then
...
if grep -q "MARKETING_VERSION" "Khandoba Secure Docs.xcodeproj/project.pbxproj"; then
    success "Marketing version configured"
else
    warning "Set MARKETING_VERSION (e.g., 1.0)"
fi

if grep -q "CURRENT_PROJECT_VERSION" "Khandoba Secure Docs.xcodeproj/project.pbxproj"; then
...
if [ -f "Khandoba Secure Docs/Configuration.storekit" ]; then
    success "StoreKit config present"
    
    if grep -q "com.khandoba.premium.monthly" "Khandoba Secure Docs/Configuration.storekit"; then
        success "Monthly subscription configured"
    else
        error "Monthly subscription missing from StoreKit"
    fi
    
    if grep -q "com.khandoba.premium.yearly" "Khandoba Secure Docs/Configuration.storekit"; then
...
# Check 6: Assets
echo "6. App Assets:"
if [ -d "Khandoba Secure Docs/Assets.xcassets/AppIcon.appiconset" ]; then
...
echo ""
echo "NEXT STEPS:"
echo "1. Run: ./scripts/prepare_for_transporter.sh"
