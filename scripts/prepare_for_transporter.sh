...
WORKSPACE="${PROJECT_ROOT}/platforms/apple/${PROJECT_NAME}.xcodeproj"
BUNDLE_ID="com.khandoba.securedocs"
TEAM_ID="Q5Y8754WU4"
BUILD_DIR="${PROJECT_ROOT}/builds/apple"
ARCHIVE_PATH="${BUILD_DIR}/archives/KhandobaSecureDocs_Final.xcarchive"
EXPORT_PATH="${BUILD_DIR}/ipas"
EXPORT_OPTIONS="${PROJECT_ROOT}/config/ios/ExportOptions.plist"
...
# Check entitlements
if grep -q "production" "${PROJECT_ROOT}/platforms/apple/Khandoba Secure Docs/Khandoba_Secure_Docs.entitlements"; then
...
for permission in "${REQUIRED_PERMISSIONS[@]}"; do
    if grep -q "$permission" "../../platforms/apple/Khandoba Secure Docs/Info.plist"; then
        echo "   ✅ $permission"
    else
...
# Change to iOS platform directory for xcodebuild
cd "${PROJECT_ROOT}/platforms/apple"
...
# Find the exported IPA (xcodebuild may create it in a subdirectory)
EXPORTED_IPA=$(find "$EXPORT_PATH" -name "*.ipa" | head -1)
if [ -f "$EXPORTED_IPA" ]; then
    echo "   ✅ IPA exported successfully"
    IPA_SIZE=$(du -h "$EXPORTED_IPA" | cut -f1)
    echo "   📦 IPA Size: $IPA_SIZE"
    # Move to expected location if needed
    if [ "$EXPORTED_IPA" != "$EXPORT_PATH/$PROJECT_NAME.ipa" ]; then
        mv "$EXPORTED_IPA" "$EXPORT_PATH/$PROJECT_NAME.ipa"
    fi
else
