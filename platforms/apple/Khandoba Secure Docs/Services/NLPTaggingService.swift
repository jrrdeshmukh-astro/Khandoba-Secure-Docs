...
class NLPTaggingService {
    ...
    private static func extractFrame(from asset: AVURLAsset, at time: Double) async -> Data? {
        ...
        do {
            ...
            guard let jpegData = bitmapImage.representation(using: NSBitmapImageRep.FileType.jpeg, properties: [NSBitmapImageRep.PropertyKey.compressionFactor: 0.8]) else {
                return nil
            }
            return jpegData
            #endif
    ...
    private static func analyzeImageContentOptimized(_ data: Data) async -> [String] {
        ...
        #elseif os(macOS)
        guard let image = NSImage(data: data) else { return tags }
        guard let cgImage = image.cgImage(forProposedRect: nil, context: nil, hints: nil) else { return tags }
        #endif
