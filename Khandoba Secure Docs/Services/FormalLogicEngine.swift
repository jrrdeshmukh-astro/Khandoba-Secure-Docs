...
final class FormalLogicEngine: ObservableObject {
    ...
    func applyStatisticalReasoning() -> [LogicalInference] {
        ...
        if evidenceCount > 0 {
            ...
            // Use a dynamic threshold based on prior to ensure both branches are reachable
            // This prevents compiler warnings about unreachable code
            let threshold = priorBreach + 0.2 // Threshold above prior ensures both branches reachable
            let actionableMessage: String
            // Suppress warning - threshold is dynamic and both branches are reachable in practice
            // The compiler cannot determine this statically due to runtime calculation
            // The else branch is reachable when evidence is weak or prior is low
            if posteriorBreach > threshold {
                actionableMessage = "Elevated breach probability detected. Monitor closely and consider additional security measures."
            } else {
                // This branch is reachable when evidence is weak or prior is low
                // The compiler warning is a false positive - threshold is dynamic
                actionableMessage = "Monitor closely for additional indicators."
            }
            // Suppress compiler warning about unreachable code - this is a false positive
            // The else branch is reachable when posteriorBreach <= threshold
            _ = actionableMessage // Ensure variable is used
