    ...
    var body: some View {
        ...
        NavigationStack {
            ...
            .alert("Approve Emergency Access?", isPresented: $showApprovalConfirmation) {
                ...
                if let request = selectedRequest {
                    Text("This will grant 24-hour read-only access to '\(request.vault?.name ?? "vault")'. The requester will be notified.")
                }
            }
    ...
    private func approveRequest(_ request: EmergencyAccessRequest) {
        Task {
            do {
                ...
                try await approvalService.approveEmergencyRequest(request, approverID: currentUser.id)
    ...
    private func denyRequest(_ request: EmergencyAccessRequest) {
        Task {
            do {
                ...
                try await approvalService.denyEmergencyRequest(request, approverID: currentUser.id)
