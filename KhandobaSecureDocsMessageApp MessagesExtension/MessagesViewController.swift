...
class MessagesViewController: MSMessagesAppViewController {
            ...
            let menuView = SimpleMenuMessageView(
                onSelectVault: { [weak self] in
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        self.presentNomineeInvitationFlow(conversation: conversation)
                    }
                },
                onTransfer: { [weak self] in
                    ...
                    self.requestPresentationStyle(.expanded)
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        self.presentTransferFlow(conversation: conversation)
                    }
                },
                onEmergency: { [weak self] in
                    guard let self = self else { return }
                    logInfo("Emergency Access selected")
                    self.requestPresentationStyle(.expanded)
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
    ...
    // MARK: - Message Creation (Simplified)
    
    private func createAndSendInvitation(vault: Vault, recipientName: String, conversation: MSConversation) {
        Task {
            do {
                // Use existing createAndSendNomination logic
                await createAndSendNomination(vault, recipientName: recipientName, conversation: conversation)
            } catch {
                await MainActor.run {
                    presentErrorView(
                        message: "Failed to send invitation: \(error.localizedDescription)",
                        onRetry: { [weak self] in
                            self?.createAndSendInvitation(vault: vault, recipientName: recipientName, conversation: conversation)
                        },
                        onCancel: { [weak self] in
                            self?.dismiss()
                        }
            ...
            do {
                ...
                await MainActor.run {
                    presentErrorView(
                        ...
                        onRetry: { [weak self] in
                            self?.createAndSendTransfer(vault: vault, recipientName: recipientName, conversation: conversation)
                        },
                        onCancel: { [weak self] in
                            self?.dismiss()
                        }
    ...
    private func createAndSendEmergency(vault: Vault, reason: String, urgency: String, conversation: MSConversation) {
        Task {
            do {
                ...
                layout.caption = "Emergency Access Request: \(vault.name)"
                layout.subcaption = reason
                message.layout = layout
                
                // Add URL with request data
                var components = URLComponents()
                components.scheme = "khandoba"
                components.host = "emergency"
                components.path = "/request"
                components.queryItems = [
                    URLQueryItem(name: "vaultID", value: vault.id.uuidString),
                    URLQueryItem(name: "requestID", value: request.id.uuidString),
                ...
                conversation.insert(message) { error in
                    if let error = error {
                        print("❌ Failed to send emergency request: \(error.localizedDescription)")
                    } else {
                ...
                await MainActor.run {
                    presentErrorView(
                        ...
                        onRetry: { [weak self] in
                            self?.createAndSendEmergency(vault: vault, reason: reason, urgency: urgency, conversation: conversation)
                        },
                        onCancel: { [weak self] in
                            self?.dismiss()
                        }
    ...
    private func presentNomineeInvitationFlow(conversation: MSConversation) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let flowView = NomineeInvitationFlowView(
            ...
            let hostingController = UIHostingController(
                rootView: flowView.environment(\.unifiedTheme, UnifiedTheme())
            )
            hostingController.view.backgroundColor = .systemBackground
            hostingController.view.translatesAutoresizingMaskIntoConstraints = false
    ...
    private func presentTransferFlow(conversation: MSConversation) {
        logInfo("Presenting Apple Cash-style transfer flow")
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let flowView = TransferOwnershipView(
                conversation: conversation,
                onCancel: { [weak self] in
                    self?.dismiss()
                },
    ...
    private func presentEmergencyFlow(conversation: MSConversation) {
        logInfo("Presenting Apple Cash-style emergency flow")
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let flowView = EmergencyAccessView(
                conversation: conversation,
                onCancel: { [weak self] in
                    self?.dismiss()
                },
                onSend: { [weak self] vault, reason, urgency in
                    self?.createAndSendEmergency(vault: vault, reason: reason, urgency: urgency, conversation: conversation)
                }
            )
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let invitationFlowView = NomineeInvitationFlowView(
                ...
                onSend: { [weak self] vault, recipientName in
                    ...
                    Task {
                        await self.createAndSendNomination(
                            vault,
                            recipientName: recipientName,
                            conversation: conversation
                        )
    ...
    private func createAndSendNomination(
        ...
        logInfo("Creating nomination for vault: \(vault.name), recipient: \(recipientName)")
