        ...
        Task {
            do {
                ...
                if let existingNominee = selectedNominee {
                    ...
                    print("✅ Resending invitation to existing nominee: \(existingNominee.name)")
                } else {
                    ...
                    print("✅ Created nominee: \(nominee.name) with token: \(inviteToken)")
                }
                ...
                print("❌ Failed to create/send invitation: \(error.localizedDescription)")
                await MainActor.run {
            ...
            do {
                ...
                let descriptor = FetchDescriptor<Nominee>(
                    predicate: #Predicate { nominee in
                        nominee.vault?.id == vaultID && nominee.status == "pending"
                    },
                ...
                print("❌ Failed to load pending nominees: \(error.localizedDescription)")
            }
        }
    }
